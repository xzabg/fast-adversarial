...
class ASDNPretrainLabelLayer(caffe.Layer):
    def setup(self, bottom, top):
        ...
        self._name_to_top_map = {
            ...
            'labels_pos': 1,
            'targets_pos': 2,
	    'inside_weights_pos': 3,
	    'outside_weights_pos': 4}
	...
	bbox_targets = np.copy(bottom[2].data)
	inside_weights = np.copy(bottom[3].data)
	outside_weights = np.copy(bottom[4].data)

        sample_num = conv_feat.shape[0]
        channels   = conv_feat.shape[1]
        pool_len   = conv_feat.shape[2]
	targets_len= bbox_targets.shape[1]
	...
	targets_pos =np.zeros((count_pos, targets_len))
	inside_weights_pos = np.zeros((count_pos, targets_len))
	outside_weights_pos = np.zeros((count_pos, targets_len))
		...
		targets_pos[cnt, :] = np.copy(bbox_targets[i, :])
		inside_weights_pos[cnt, :] = np.copy(inside_weights[i, :])
		outside_weights_pos[cnt, :] = np.copy(outside_weights[i, :])
                cnt = cnt + 1
        ...
        top[2].reshape(*(targets_pos.shape))
        top[2].data[...] = targets_pos.astype(np.float32, copy=False)
	top[3].reshape(*(inside_weights_pos.shape))
	top[3].data[...] = inside_weights_pos.astype(np.float32, copy=False)
	top[4].reshape(*(outside_weights_pos.shape))
	top[4].data[...] = outside_weights_pos.astype(np.float32, copy=False)
...
class ASDNPretrainDataLayer(caffe.Layer):
    def setup(self, bottom, top):
        ...
        self._name_to_top_map = {
            'conv_feat_rep': 0,
	    'labels_final': 1,
	    'bbox_targets_final': 2,
	    'inside_weights_final': 3,
	    'outside_weights_final': 4}
    ...
    def generate_feature(self, conv_feat, labels, bbox_targets, inside_weights, outside_weights):

        sample_num = conv_feat.shape[0]
        channels   = conv_feat.shape[1]
        pool_len   = conv_feat.shape[2]

	final_len  = bbox_targets.shape[1]
	...
	outside_weights_final = np.zeros((sample_num * rep_num_area, final_len))
	...
	outside_weights_final[cnt * sample_num : cnt * sample_num + sample_num, :] = np.copy(outside_weights)
        ...
        return conv_feat_rep, labels_final, bbox_targets_final, inside_weights_final, outside_weights_final
	...
	conv_feat_rep, labels_final ,bbox_targets_final ,inside_weights_final ,outside_weights_final=self.generate_feature(conv_feat,labels,bbox_targets,inside_weights,outside_weights)

        top[0].reshape(*(conv_feat_rep.shape))
        top[0].data[...] = conv_feat_rep.astype(np.float32, copy=False)

	top[1].reshape(*(labels_final.shape))
	top[1].data[...] = labels_final.astype(np.float32, copy=False)

	top[2].reshape(*(inside_weights_final.shape))
	top[2].data[...] = inside_weights_final.astype(np.float32, copy=False)

	top[3].reshape(*(outside_weights_final.shape))
	top[3].data[...] = outside_weights_final.astype(np.float32, copy=False)

	top[4].reshape(*(outside_weights_final.shape))
	top[4].data[...] = outside_weights_final.astype(np.float32, copy=False)
